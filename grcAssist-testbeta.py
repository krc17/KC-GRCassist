...
def write_to_spreadsheet(articles, filename, keyword):
    """
    Writes a list of articles to a spreadsheet file, skipping duplicates.
    Deduplication is based on title + description.
    """
    today = datetime.date.today().strftime("%Y-%m-%d")

    # Load existing title+description pairs
        ...
        with open(filename, 'r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                if len(row) >= 5:
                    ...
                    existing_entries.add((title, desc))
    ...
    with open(filename, 'a', newline='', encoding='utf-8') as csvfile:
        ...
        for article in articles:
            title = (article["headline"] or "").strip().lower()
            desc = (article["description"] or "").strip().lower()

            entry_key = (title, desc)
